...
fn require_user(state: &AppState, headers: &HeaderMap) -> Result<store::UserRow, Response> {
    bearer(headers)
        ...
        .ok_or_else(|| fail(StatusCode::UNAUTHORIZED, "unauthorized", "Sign in to continue."))
}
...
async fn register(State(state): State<AppState>, body: String) -> Response {
    let Ok(body) = serde_json::from_str::<serde_json::Value>(&body) else {
        return fail(StatusCode::BAD_REQUEST, "bad_json", "Request body was not valid JSON.");
    };
    ...
    if password.chars().count() < 8 {
        return fail(StatusCode::BAD_REQUEST, "invalid", "Password must be at least 8 characters.");
    }
    if password.chars().count() > 256 {
        return fail(StatusCode::BAD_REQUEST, "invalid", "Password is too long.");
    }
    ...
    match store::create_user(&state.db, username, &display, password) {
        store::CreateUser::Taken => {
            fail(StatusCode::CONFLICT, "taken", "That username is already taken.")
        }
        store::CreateUser::Created(id) => {
            let Some(row) = store::find_by_id(&state.db, &id) else {
                return fail(StatusCode::INTERNAL_SERVER_ERROR, "internal", "Could not create the account.");
            };
...
async fn file_init(State(state): State<AppState>, headers: HeaderMap, body: String) -> Response {
    ...
    if name.is_empty() {
        return fail(StatusCode::BAD_REQUEST, "bad_request", "A file name is required.");
    }
    if size < 0 {
        return fail(StatusCode::BAD_REQUEST, "bad_request", "Invalid file size.");
    }
    ...
    if !store::user_exists(&state.db, to) {
        return fail(StatusCode::NOT_FOUND, "no_such_user", "That recipient no longer exists.");
    }

    match files::init_upload(&state.db, &state.data_dir, &user.id, name, size, mime, to) {
        Err(_) => fail(StatusCode::INTERNAL_SERVER_ERROR, "internal", "Could not start the upload."),
        Ok(file_id) => (
            StatusCode::CREATED,
...
async fn file_chunk(
    ...
    let Some(file) = files::get(&state.db, &file_id) else {
        return fail(StatusCode::NOT_FOUND, "not_found", "Unknown upload.");
    };
    if file.owner_id != user.id {
        return fail(StatusCode::FORBIDDEN, "forbidden", "That upload belongs to someone else.");
    }
    if file.complete {
        return Json(json!({
            "complete": true,
...
async fn file_download(
    ...
    let Some(user) = store::resolve_session(&state.db, &token) else {
        return fail(StatusCode::UNAUTHORIZED, "unauthorized", "Sign in to continue.");
    };
    let Some(file) = files::get(&state.db, &file_id) else {
        return fail(StatusCode::NOT_FOUND, "not_found", "That file is no longer available.");
    };
    if file.owner_id != user.id && file.to_id != user.id {
        return fail(StatusCode::FORBIDDEN, "forbidden", "That file was not shared with you.");
    }
    if !file.complete {
        return fail(StatusCode::CONFLICT, "incomplete", "That file is still uploading.");
    }

    match files::read_blob(&state.data_dir, &file) {
        Err(_) => fail(StatusCode::NOT_FOUND, "not_found", "That file is no longer available."),
        Ok(bytes) => {
            ...
            Response::builder()
                ...
                .unwrap_or_else(|_| {
                    fail(StatusCode::INTERNAL_SERVER_ERROR, "internal", "Could not send the file.")
                })
        }
    }
}
        ...
        ClientEvent::Unknown => {
            state.hub.send_to_socket(
                ...
                &ServerEvent::Error {
                    ...
                    message: "Unrecognised event.".into(),
                },
            );
        }
    }
    ...
    if !store::user_exists(&state.db, &to) {
        state.hub.send_to_socket(
            ...
            &ServerEvent::Error {
                ...
                message: "That person is no longer in the directory.".into(),
            },
        );
        return;
    }
        ...
        if !usable {
            state.hub.send_to_socket(
                ...
                &ServerEvent::Error {
                    ...
                    message: "That file is not ready to send.".into(),
                },
            );
            return;
        }
