...
import type { Attachment, Message, ServerEvent, User, UserId } from "@comms/protocol";
...
const EMPTY: State = { self: null, users: [], threads: {}, unread: {}, typing: {}, ready: false };
...
function applyEvent(state: State, { event, activePeer }: Extract<Action, { type: "event" }>): State {
  switch (event.t) {
    case "ready": {
      ...
      return { self: event.self, users: event.users, threads, unread, typing: {}, ready: true };
    }
