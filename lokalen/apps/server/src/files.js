...
import { CHUNK_SIZE, MAX_FILE_BYTES, chunkCountFor } from "@lokalen/protocol";
...
export async function initUpload(db, dataDir, ownerId, { name, size, mime, to }) {
  if (typeof name !== "string" || !name.trim()) throw new HttpError(400, "bad_request", "Ett filnamn krävs.");
  if (!Number.isInteger(size) || size < 0) throw new HttpError(400, "bad_request", "Ogiltig filstorlek.");
  if (size > MAX_FILE_BYTES) throw new HttpError(413, "too_large", "Filen är större än gränsen på 2 GB.");

  const recipient = db.prepare("SELECT id FROM users WHERE id = ?").get(to);
  if (!recipient) throw new HttpError(404, "no_such_user", "Mottagaren finns inte längre.");
...
export async function writeChunk(db, dataDir, file, index, buffer) {
  if (!Number.isInteger(index) || index < 0 || index >= file.chunk_count) {
    throw new HttpError(400, "bad_chunk", "Segmentindex utanför intervallet.");
  }
  const expected = index === file.chunk_count - 1 ? file.size - index * CHUNK_SIZE : CHUNK_SIZE;
  if (buffer.length !== expected) {
    throw new HttpError(400, "bad_chunk", `Segment ${index} skulle vara ${expected} byte, fick ${buffer.length}.`);
  }
